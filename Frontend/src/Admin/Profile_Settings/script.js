...
async function renderInformation(){
    ...
    adminid.placeholder = data.adminId
    ...
    joiningdate.value = joindate.toISOString().split('T')[0] ;
...
async function updateInformation(){
    ...
    const url = `${API_DOMAIN}/admin/updateInformation`;
...
async function removeProfilePicture(){
    const url = `${API_DOMAIN}/admin/removeProfilePicture`;
    try{
        const response = await fetch(url,
        {
            method : "GET",
        ...
        updateProfileImages(false, true);
    }
    catch(error){
...
async function changePassword(){
    ...
    const url = `${API_DOMAIN}/admin/changePassword`
    const response = await fetch(url,
    {
        method : "POST",
        credentials : 'include',
